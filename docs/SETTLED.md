...
| **The CRD catalog's output is byte-reproducible.** `KIND_RANK` values are unique and the tie-break sorts on `(rank, name)` — a tie previously resolved by set-iteration order, which varies between Python processes and churned the committed CSV | verified 2026-09-03: repeated full 8,784-file runs byte-identical, and identical to the committed CSV |
...
| **Stage 3 attributes 89.3% of 471,878 archive RINEX files to a monument from the header position alone.** 735 filename conflicts, 1,017 directory-path conflicts | `docs/bern52/rinex_attribution.md`, 2026-09-05 |
| **~18,200 unattributable files are PAGENET (NAMRIA) stations, not a matcher failure.** 40 codes the catalog has never held, 28 of them `P`-prefixed, in `RAW/2018/Pagenet_*`. Median 8 km from the nearest catalog site | the catalog is built from PHIVOLCS `.crd`; NAMRIA coordinates would be needed to attribute them |
| **A glob list is the wrong tool for file selection over this archive.** Three attempts read 84,198 then 443,195 then 471,878 files; each omission was silent, because a glob matching nothing looks like a corpus containing nothing | use one case-insensitive pattern stating the rule |
...
| **A negative is evidence only from an instrument you have confirmed was armed.** Check the instrument BEFORE reading its silence | the two poles, both 2026-09: **sysrq** was NOT armed — `kernel.sysrq = 438` masked off `S` and `B`, so REISUB doing nothing on a hung finch meant nothing, and a healthy kernel would have been equally silent. **pstore** WAS armed — `efi_pstore` registered on all three crash boots, verified against the committed journals, and `/sys/fs/pstore` empty. Same silence, opposite worth: the second eliminated kernel panic outright and took the live hypotheses from four to two. The check costs one command |
...
| **`CRX2RNX` is at `~/BERN54/SCRIPT/EXE/CRX2RNX` on gps3, and whether it is on `PATH` depends on the SHELL, not on the user** — a login shell has it, because the profile sources `~/BERN54/LOADGPS.setvar`; a non-login shell does not. So an interactive terminal finds it and a cron job, a systemd unit or any non-login script does not. The setvar is at `~/BERN54/LOADGPS.setvar`, not under `GPS/EXE/` | verified 2026-09-17 by existence (`test -x`) and by a CLEAN shell. `bash -c` is not a clean shell — it inherits the caller's already-configured `PATH` and reported the tool present, which would have "disproved" a true claim. The clean check is `env -i /bin/bash --noprofile --norc`. Same lesson as the row above, one level up: a PATH test run in a contaminated environment |
...
| **Do not deduplicate the recovery archive** | which drive held which copy is itself the evidence `drive-archaeologist` exists to recover. Group duplicates and report them; never prune. Disk is not scarce (3.7 TB free) and 26% of same-name pairs differ in bytes |
