...
5a. **Capture `PAGENET_DLY.PCF` from the T420** into `config/bernese/gpsuser/PCF/`. It exists nowhere else. **Do not re-derive it**: truncating RNX2SNX at PID 514 leaves `599 DUMMY` waiting on `522`, a dangling WAIT that hangs the BPE forever — and a re-derived PCF would not be the one the training week validated.
