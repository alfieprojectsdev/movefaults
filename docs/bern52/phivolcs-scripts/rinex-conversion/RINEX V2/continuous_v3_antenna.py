...
print('Running: continuous_v3_antenna.')
...
def start():    #while defining, the program does not yet recognize it unless called
    ...
    def runpk():
        ...
        for rnx in glob.glob('*.t0*'):
                os.system('runpkr00 -d ' + rnx) #.t0 files are now converted to .dat
                print("        Converting " + rnx + " to .DAT...")
    ...
    def rinex():
        mo = input('\nSite name: ')
        mo = mo.upper()
        for teqc in glob.glob('*.dat'):
            os.system('teqc -tr d -O.dec 30s -O.r ' +person+' -O.mo '+mo+' -O.mn '+mo+' -O.at "TRM57971.00" -O.pe 0 0 0 +obs + -tbin 1d '+mo+' '+teqc)
    ...
    def leica():
        ...
        for teqc in glob.glob('*.'+fe+'??'):
            os.system('teqc -phc -O.dec 30s -O.s GPS -O.r LordP -O.mo '+mo+' -O.mn '+mo+' -O.pe 0 0 0 +obs + -tbin 1d '+mo+' '+teqc)
