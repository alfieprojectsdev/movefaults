...
def start():    #while defining, the program does not yet recognize it unless called
    ...
    def rinex():
        ...
        for teqc in glob.glob('*.tgd') or glob.glob('*.dat'):
            os.system('teqc -tr d -O.dec 30s -O.r ' +person+' -O.mo '+mo+' -O.mn '+mo+' -O.pe 0 0 0 +C2 +obs + -tbin 1d '+mo+' '+teqc) #added +C2 on 12.09.2020
    ...
    def leica():
        ...
        for teqc in glob.glob('*.'+fe+'??'):
            os.system('teqc -phc -O.dec 30s -O.r ' +person+' -O.mo '+mo+' -O.mn '+mo+' -O.pe 0 0 0 +C2 +obs + -tbin 1d '+mo+' '+teqc) #added +C2 for Bernese 5.2 processing
    ...
    flow = input('Input action. \n1 = start runpkr00, 2 = Trimble teqc, 3 = Leica teqc, 4 = Trimble converter\n')
    if flow == '1':
        runpk()
    elif flow == '2':
        print('\nTRIMBLE TEQC')
