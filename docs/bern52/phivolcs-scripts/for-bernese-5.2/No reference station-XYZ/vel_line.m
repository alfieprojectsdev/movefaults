%% plot time series figure and calculate velocity simpally (fitting a straingh line)%%
...
for k=1:1:num_file
    ...
    sitename=files;x=d(:,1);y=d(:,2);z=d(:,3);
    d=d*100;  %m--> cm
    N=length(t);
    ...
    figure;
    hx=subplot(3,1,1);
    plot(t,d(:,1),'bo',t,dhat(:,1)','g-');ylabel('X (cm)');title(sitename);
    %gtext(['V=' num2str(model(2,1)*10,2) ' cm/yr']);
    legend(['V=' num2str(model(2,1)*10,2) ' mm/yr']);
    set(hx,'linewidth',1.2,'box','on','fontsize',12);
    
    hy=subplot(3,1,2);
    plot(t,d(:,2),'bo',t,dhat(:,2)','g-');ylabel('Y (cm)');
    %gtext(['V=' num2str(model(2,2)*10,2) ' cm/yr']);
    legend(['V=' num2str(model(2,2)*10,2) ' mm/yr']);
    set(hy,'linewidth',1.2,'box','on','fontsize',12);
    
    hz=subplot(3,1,3);
    plot(t,d(:,3),'bo',t,dhat(:,3)','g-');ylabel('Z (cm)');xlabel('TIME');
    %gtext(['V=' num2str(model(2,3)*10,2) ' cm/yr']);
    legend(['V=' num2str(model(2,3)*10,2) ' mm/yr']);
    set(hz,'linewidth',1.2,'box','on','fontsize',12);
