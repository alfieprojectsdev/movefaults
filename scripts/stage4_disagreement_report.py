...
def main() -> int:
    ...
    def truncated(n_shown: int, total: int, what: str) -> None:
        """Say what the table left out. A report that silently shows the top 15
        of 300 understates the work, and the reader cannot tell."""
        if total > n_shown:
            A(f"\n*Showing {n_shown} of {total} {what}. "
              f"{total - n_shown} not listed — raise `--top` to see them.*\n")
    ...
    truncated(min(args.top, len(amb_groups)), len(amb_groups), "ambiguous-code patterns")
    ...
    truncated(min(args.top, len(clean_groups)), len(clean_groups), "unambiguous-code patterns")
    ...
    truncated(min(args.top, len(path_groups)), len(path_groups), "path patterns")
    ...
    for d, s in multi[:args.top]:
        ...
        A(f"| `…/{short}` | {len(s)} — {' '.join(sorted(s)[:6])}"
          f"{' …' if len(s) > 6 else ''} |")
    truncated(min(args.top, len(multi)), len(multi), "multi-site directories")
