...
# NAMES ARE PHREF_-PREFIXED, and the defaults do NOT read the bare names.
# LOADGPS.setvar exports a large set of short uppercase variables -- P D U C T
# S SRC PCF PAN OPT and more. `PCF=PHREF_DLY` became $U/PCF that way in
# run_luzon_month.sh, and it happened again here: writing
# `PCF="${PCF:-PHREF_DLY}"` lets an already-polluted environment supply the
...
PHREF_YEAR="${PHREF_YEAR:-2025}"
PHREF_PCF="${PHREF_PCF:-PHREF_DLY}"
PHREF_MAXSESS="${PHREF_MAXSESS:-6}"
LOG_DIR="$HOME/phref-year-logs"

# Days that must never be attempted, for the reasons above.
# Re-derived for THIS network, not inherited. The LUZON list excluded days
# for having too few LUZON reference stations; PHREF has a different
# station set, so 079 and 139 are fine here and other days are not.
PHREF_EXCLUDE="058 059 060 061 345"
...
compute_blocks() {
    ...
    for d in $(seq 1 365); do
        dd=$(printf '%03d' "$d")
        want=yes
        case " $PHREF_EXCLUDE " in *" $dd "*) want=no ;; esac
        [ "$want" = yes ] && ls "$S/PHREF/$PHREF_YEAR/SOL/FIN_${PHREF_YEAR}${dd}0."* \
...
for b in $BLOCKS; do
    ...
    while [ "$start" -le "$to" ]; do
        ...
        perl "$DRV" "$PHREF_YEAR" "$sess" "$PHREF_PCF" "$n_left" "$PHREF_MAXSESS" \
            >>"$log" 2>&1
        ...
        for d in $(seq "$start" "$to"); do
            ls "$S/PHREF/$PHREF_YEAR/SOL/FIN_${PHREF_YEAR}$(printf '%03d' "$d")0."* \
