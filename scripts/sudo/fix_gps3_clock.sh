...
# ONE definition, used by the one-shot below AND injected into the timer it
# installs. They were separately defined and drifted: the timer allowed 15s of
# spread while this refused at 5s, so re-running the fix aborted on a spread
# the installed timer would have accepted. Two thresholds for one decision.
#
# 15s: an HTTP Date is second-resolution plus request latency, and github runs
# a consistent ~9s behind cloudflare and google -- slow, not wrong. A source
# that is genuinely wrong is wrong by minutes, as this host was by 250s.
SPREAD_MAX=15
...
cat <<INNERCFG
#!/usr/bin/env bash
# GENERATED by fix_gps3_clock.sh -- do not edit; edit that script instead.
SPREAD_MAX=${SPREAD_MAX}
SRC=(${SOURCES[*]})
INNERCFG
cat <<'INNER' ; } > /usr/local/sbin/http-timesync
