import os, glob, math
print('Running: campv5_combine.exe')
print('---------------------------------------------------------------------------')
...
print('NOTE: For Input file name, use asterisk(*) to pass multiple files.')
...
def start():    #while defining, the program does not yet recognize it unless called
    ...
    def rinex():
        ...
        while code == 1:
            ...
            for teqc in glob.glob(files):
                os.system('teqc -tr d -O.dec 30 -phc -O.mo "' + mo + '" -O.at ' + at + ' -O.pe ' + ("%.4f" %pe1) + ' 0 0 +C2 +obs + -tbin 1d ' + mo+' '+teqc) #added +C2 on 12.09.2020
