...
def start(n_samples=10):
    ...
    for i in range(n_samples):
        #set sample size
        def create_boothstrap_samples(sample_size=12):

            #draw a bootstrap sample of size n
            bootstrap_sample=df.sample(sample_size, replace=True)
            
            #add the skipped row back
            df1=pd.DataFrame({"sites":["IBAZ"],"Ecoord":[42173.1739],"Ncoord":[-919208.7877],"height":[-67087.8326],"velocity":[0],"azimuth":[0],"Vu":[0],"hgt":[0],"erreast":[0],"errnorth":[0]},index=[13])
            bootstrap_sample=bootstrap_sample.append(df1)
            
            print(bootstrap_sample)
    
            #write samples in text
            bootstrap_sample.to_csv('PHm.txt', sep='\t', index=False)
