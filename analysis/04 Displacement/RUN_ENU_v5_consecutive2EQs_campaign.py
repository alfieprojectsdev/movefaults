...
def regressline():
	...
	e = (e - numpy.mean(all_e)) * 100
	n = (n - numpy.mean(all_n)) * 100
	u = (u - numpy.mean(all_u)) * 100
	seismic_e = (seismic_e - numpy.mean(all_e)) * 100
	seismic_n = (seismic_n - numpy.mean(all_n)) * 100
	seismic_u = (seismic_u - numpy.mean(all_u)) * 100
	eq_e = (eq_e - numpy.mean(all_e)) * 100
	eq_n = (eq_n - numpy.mean(all_n)) * 100
	eq_u = (eq_u - numpy.mean(all_u)) * 100
	all_e = (all_e - numpy.mean(all_e)) * 100
	all_n = (all_n - numpy.mean(all_n)) * 100
	all_u = (all_u - numpy.mean(all_u)) * 100
	...
	for event2 in eq_t:        
		...
		j = -1
		k = -1
		predict2_e = (me*event2)+be2
		predict2_n = (mn*event2)+bn2
		predict2_u = (mu*event2)+bu2
