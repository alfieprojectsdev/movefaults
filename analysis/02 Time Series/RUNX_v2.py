...
def start(reference_station=None):
    # When reference_station is provided (headless mode), skip all input() prompts.
    interactive = reference_station is None
    ...
    def transform():
        ...
        if reference_station is not None:
            refsite = reference_station.upper()
        else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='RUNX v2 — Transform Bernese CRD files to ENU and create PLOT files'
    )
    parser.add_argument(
        '--reference-station', '-r',
        metavar='CODE',
        help='4-char station code to use as ENU reference origin (omit for interactive prompt)',
    )
    args = parser.parse_args()
    start(reference_station=args.reference_station)
